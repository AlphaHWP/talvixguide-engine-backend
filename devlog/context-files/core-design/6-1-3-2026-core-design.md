...
You are now receiving the core architectural context for the Talvixguide Engine.
...
The system supports:
- Creator-controlled metadata publishing
- Decentralized trust via cryptographic signatures
- Pluggable discovery engines
- Cross-platform client tools

## Major Components & Responsibilities
- **Backend Engine**: Core ingestion, parsing, and indexing pipeline (C++23/26)
- **Signer Tool**: CLI/GUI for creators to sign TOML metadata (Qt6)
- **Extractor Tool**: Pulls metadata from platforms (e.g., YouTube API) → TOML
- **Frontend Clients**: User-facing apps for content discovery (future)
- **Metadata Store**: Organized in `/metadata/raw_toml` and `/metadata/processed_json`

## Component Interactions & Data Flow
1. **Creator Workflow**:
   - Extractor → YouTube API → TOML → Signer → Signed TOML → Export
2. **Ingestion Workflow**:
   - Signed TOML → Parse (toml++) → Convert to JSON → Verify (libsodium) → Index (yyjson)
3. **Discovery Workflow**:
   - Indexed JSON → Pluggable recommenders → Frontend
...
Acknowledge this prompt with: "Core design context received. Awaiting work units and roadmap."   
